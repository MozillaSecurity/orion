...
def test_create_02(mocker: MockerFixture) -> None:
    ...
    assert task == yaml_load(
        BUILD_TASK.substitute(
            ...
            route="project.fuzzing.orion.test1.main",
...
def test_create_04(mocker: MockerFixture) -> None:
    ...
    expected2 = yaml_load(
        BUILD_TASK.substitute(
            ...
            owner_email=OWNER_EMAIL,
            provisioner=PROVISIONER_ID,
            route="project.fuzzing.orion.test2.main",
            scheduler=SCHEDULER_ID,
            service_name="test2",
            source_url=SOURCE_URL,
            task_group="group",
...
def test_create_07(mocker: MockerFixture) -> None:
    ...
    assert task == yaml_load(
        BUILD_TASK.substitute(
            ...
            owner_email=OWNER_EMAIL,
            provisioner=PROVISIONER_ID,
            route="project.fuzzing.orion.test1.pull_request.1",
            scheduler=SCHEDULER_ID,
            service_name="test1",
            source_url=SOURCE_URL,
            task_group="group",
...
def test_create_08(
    ...
    if ci1_dirty:
        ...
        assert task1 == yaml_load(
            BUILD_TASK.substitute(
                ...
                owner_email=OWNER_EMAIL,
                provisioner=PROVISIONER_ID,
                route="project.fuzzing.orion.testci1.main",
                scheduler=SCHEDULER_ID,
    ...
    expected2 = yaml_load(
        TEST_TASK.substitute(
            ...
            owner_email=OWNER_EMAIL,
            provisioner=PROVISIONER_ID,
            route=f"project.fuzzing.orion.{svc}.main",
            scheduler=SCHEDULER_ID,
            service_name=svc,
            source_url=SOURCE_URL,
            task_group="group",
...
def test_create_09(mocker: MockerFixture) -> None:
    ...
    assert task1 == yaml_load(
        BUILD_TASK.substitute(
            ...
            owner_email=OWNER_EMAIL,
            provisioner=PROVISIONER_ID,
            route="project.fuzzing.orion.test5.main",
            scheduler=SCHEDULER_ID,
            service_name="test5",
            source_url=SOURCE_URL,
            task_group="group",
    ...
    expected3 = yaml_load(
        BUILD_TASK.substitute(
            ...
            owner_email=OWNER_EMAIL,
            provisioner=PROVISIONER_ID,
            route="project.fuzzing.orion.test6.main",
            scheduler=SCHEDULER_ID,
            service_name="test6",
            source_url=SOURCE_URL,
            task_group="group",
