...
retry python3 -m pip install --user --no-build-isolation fuzzfetch git+https://github.com/MozillaSecurity/site-scout
...
python3 -m site_scout ./build/firefox -i "$yml" --status-report status.txt --time-limit 60 --jobs 2
