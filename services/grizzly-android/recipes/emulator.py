...
def avd():
    # create an avd
    avd_ini = os.path.join(avd_path, avd_name + ".ini")
    assert not os.path.isfile(avd_ini)
    avd_dir = os.path.join(avd_path, avd_name + ".avd")
    os.mkdir(avd_dir)
...
def firstboot():
    ...
    with open(os.devnull, "w") as devnull:
        ...
        subprocess.check_output([os.path.join(platform_tools, 'adb'), 'wait-for-device', 'shell', 'while [[ -z $(getprop sys.boot_completed) ]]; do sleep 1; done'])
        time.sleep(5)
        proc.terminate()
        proc.wait()
...
def main():
    ...
    aparse.add_argument('actions', nargs='*', choices=['install', 'avd', 'firstboot'])
